const taxInfoData = {
  ...
  nonInvoicedExpenses: {
    ...
    risk: "无票支出过高或缺乏充分证据，可能导致税前扣除被否认，���加税务风险"
  },
  totalAssets: {
    title: "资产总额",
  ...
  educationExpenses: {
    title: "教育经费",
    description: "企业发生的职工教育和培训相关支出，包括内部培训费用、外部培训费用、教育资料费、培训场地费等各项教育培训相关支出",
    analysis: `职工教育经费支出政策分析：
1. 基本扣除标准：不得超过工资薪金总额的8%
2. 结转规定：超过部分准予结转下一年度扣除
3. 特殊行业：高新技术企业可按不超过工资薪金总额的12%扣除
4. 疫情期间：线上培训费用可据实扣除
5. 支出范围：包括学历教育、岗位培训、技能提升等相关费用`,
    risk: `教育支出主要风险提示：
1. 支出构成不合理或缺乏真实性，可能引起税务检查
2. 超标准列支且未合理结转的部分将被调整
3. 培训费用与企业主营业务无关可能被认定为不得扣除
4. 虚构培训支出或虚开培训发票的行为将受到严惩
5. 未保存完整的培训记录和考核资料可能影响扣除`
  },
  insuranceExpenses: {
    title: "保险费",
